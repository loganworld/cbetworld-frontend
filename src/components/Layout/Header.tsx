...
const Header = () => {
	...
	const [status, setStatus] = React.useState<HeaderInterface>({
        ...
        withdrawAddress: '',
        withdrawAmount: '',
        depositAddress: '0xE71E33e78C5d4764549123Ec4B770eF66FbD2398'
	})
	...
	return (
        <StyledHeader>
            <header>
                ...
                <div className="container">
                    <div className="justify">
                        <div></div>
                        <div className='balance' style={{display: window.innerWidth > 1280 ? 'flex': 'none'}}>
                        ...
                        {/* <div className="flex middle">
                            ...
                            <Link to ="/" className='ml1 mr1'> <Icon icon = "Chat" size={19}/> </Link>
                        </div> */}
                        <div className="flex center" style={{display: window.innerWidth > 768 ? 'flex': 'none'}}>
                ...
                </div>
                {
                    status.showMenuModal && <StyledModal>
                        ...
                        <div className="modal-container">
                            ...
                            <div>
                                {/* <div className="flex middle center mt3">
                                    <Link to ="/" className='mr3'> <Icon icon = "User" /> </Link>
                                    <Link to ="/" className='ml1 mr1'> <Icon icon = "Chat" size={19}/> </Link>
                                </div> */}
			...
			<Modal onClose = {() => {updateStatus({showRegisterModal: false})}} show={status.showRegisterModal} >
                ...
                <p className='mt3 mb1'>Password</p>
                <input type="password" style={{width: '100%', fontSize: '18px'}}  value={status.signuppassword} onChange={(v) => {updateStatus({signuppassword: v.target.value})}}/>
                <button className="btn mt3" style={{backgroundColor: '#1F8A00'}}>PLAY NOW</button>
			...
			<Modal onClose = {() => {updateStatus({showLoginModal: false})}} show={status.showLoginModal} >
                ...
                <p className='mt3 mb1'>Email/Username *</p>
                <input type="text" style={{width: '100%', fontSize: '18px'}} value={status.loginmail} onChange={(v) => {updateStatus({loginmail: v.target.value})}}/>
				
                <p className='mt3 mb1'>Password</p>
                <input type="password" style={{width: '100%', fontSize: '18px'}}  value={status.loginpassword} onChange={(v) => {updateStatus({loginpassword: v.target.value})}}/>
				
                <button className="btn mt3" style={{backgroundColor: '#1F8A00'}}>PLAY</button>
				...
				</div>
                {
                    status.walletTab === 1 && <>
                        <p className='mt3 mb1'>Address</p>
                         <input type="text" style={{width: '100%', fontSize: '18px'}} value={status.depositAddress} onChange={(v) => {updateStatus({depositAddress: v.target.value})}} readOnly/>
                        <button className="btn mt3 mb5" style={{backgroundColor: '#1F8A00'}} onClick={() => {copyToClipboard(status.depositAddress)}}>COPY ADDRESS</button>
                    </>
                }
                { 
                    status.walletTab === 2 && <>
